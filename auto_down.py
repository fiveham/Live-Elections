...
class AutoDown:
  def __init__(self, date_string, countyIDs, repo_path, cache_path='./cache/', simulator=None):
    if cache_path and simulator:
        raise Exception("Simulating by reading a cache and then also writing into that cache would be a bad idea.")
  ...
  def upload(self, state, county, prec):
    ...
    subprocess.run("git add .".split(),
                   cwd=self.repo_path)
    subprocess.run(['git', 'commit', '-m', "upload %d-%d-%d results"%fill_blanks()],
                   cwd=self.repo_path)
  ...
  def run(self):
    ...
    while True:
      ...
      if state_results != prev_state_results:
        ...
        counties = {county:self.getter.get_county(county) for county in self.countyIDs}
        self.cache_it("counties",counties)

        precincts = {county:self.getter.get_precincts(county) for county in self.countyIDs}
...
class FromCache:
  def __init__(self, cache_dir, sim_start):
    ...
    #When this simulation starts, it will pretend that that moment is the moment represented by sim_start
    self.sim_start = sim_start
    self.op_start = None #The actual moment in time when this simulation begins running
    #TODO sort as datetime.datetime objects then convert to tuples
    self.cache_minutes = [tuple(int(n)
  ...
  def before(self, dhm):
      if dhm in self.cache_minutes:
        ...
        return next(iter(minute for minute in reversed(self.cache_minutes) if minute < dhm),
  ...
  def get_county(self, countyID):
    ...
    with open(self.cache_dir + "counties_%d-%d-%d.txt" % cache_dhm, 'r') as outof:
...
def dry_run():
  ...
  sim = FromCache(cache_read, datetime.datetime(2019, 4, 30, 19, 29)) #start simulation from 7:29PM, before polls even closed
